...
public class Email {
	...
	public Email(String email) {
		...
		if ( ! email.matches("^[\\w-_\\.]+@([\\w-]+\\.)+[\\w-]{2,4}$")) {
			...
			throw new EmailNotValidException("The email format was not adequate, before the @ we allow the use of -_. and - after the @");
		}
		
		if( email.length()>=255 ) {
			throw new LongInputException("The email length was too much, if saved, it has been truncated.");
		}
